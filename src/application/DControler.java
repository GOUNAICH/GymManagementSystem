...
public class DControler implements Initializable {
	...
	public void revenueChart() {
        ...
        String sql = "SELECT startDate, SUM(amount) AS TotalPaid "
                     + "FROM members WHERE status = 'paid' "
	...
	/*
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void initMemberChart() {
		// Initialize MembersChart
	...
	private void initRevenueChart() {
		// Initialize RevenueChart
		XYChart.Series series = new XYChart.Series();
		series.getData().add(new XYChart.Data("0", 0));
		series.getData().add(new XYChart.Data("1", 30));
		series.getData().add(new XYChart.Data("2", 35));
